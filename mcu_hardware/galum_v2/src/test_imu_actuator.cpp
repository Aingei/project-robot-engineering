// ============================================================================
// ESP32 micro-ROS: 1 Stepper, 4 Servos, 2 Ultrasonics, 1 IMU (Integrated Version)
// ============================================================================
...
#include <std_msgs/msg/float32.h> // ⭐️ เพิ่มสำหรับส่งค่ามุม Yaw
...
#include "../lib/imu/imu_bno055.h"           // ⭐️ Include Library ของคุณ
...
rcl_timer_t     imu_timer;     // ⭐️ Timer สำหรับ IMU
...
void imuTimerCallback(rcl_timer_t *timer, int64_t last_call_time); // ⭐️ Callback ของ IMU
...
void servoCallback(const void *msgin) {
  ...
  double deg4 = max(0.0, min(180.0, m->angular.x)); 
...
// ⭐️ เพิ่ม Callback ดึงค่ามุมและ Publish ออกไปให้ ROS
void imuTimerCallback(rcl_timer_t *, int64_t) {
    if(bno_initialized) {
        float roll, pitch, yaw;
        if(myImu.getAngle(roll, pitch, yaw)) {
            imu_msg.data = yaw; 
...
void setup() {
  ...
  Wire.begin(); // เตรียม I2C

  // ⭐️ เรียกใช้งาน IMU ด้วย Class ของคุณ
  if(!myImu.init()) {
...
bool createEntities() {
  ...
  // ⭐️ สร้าง Publisher ตัวใหม่ให้ IMU
  RCCHECK(rclc_publisher_init_best_effort(&imu_publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32), "/galum/imu/yaw"));
  ...
  // ⭐️ สร้าง Timer ให้ดึงและส่งค่า IMU ทุกๆ 50ms
  RCCHECK(rclc_timer_init_default(&imu_timer, &support, RCL_MS_TO_NS(50), imuTimerCallback));

  us_msg.data.capacity = 2; us_msg.data.size = 2; us_msg.data.data = us_data_buffer;

  executor = rclc_executor_get_zero_initialized_executor();
  
  // ⭐️ เปลี่ยน handles จาก 4 เป็น 5 (เพราะเรามี Timer เพิ่มมา 1 ตัว)
  ...
  // ⭐️ โยน Timer ของ IMU ให้ Executor รับไปรัน
...
bool destroyEntities() {
  ...
  RCRET_IGNORE(rcl_timer_fini(&us_timer));
  RCRET_IGNORE(rcl_timer_fini(&imu_timer)); // ⭐️ ทำลาย Timer ทิ้ง
  ...
  RCRET_IGNORE(rcl_publisher_fini(&servo_status_pub, &node));
  RCRET_IGNORE(rcl_publisher_fini(&us_publisher, &node));
  RCRET_IGNORE(rcl_publisher_fini(&imu_publisher, &node)); // ⭐️ ทำลาย Publisher ทิ้ง
