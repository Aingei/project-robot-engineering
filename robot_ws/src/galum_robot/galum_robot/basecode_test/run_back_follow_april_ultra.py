...
class RobotMaster(Node):
    def __init__(self):
        ...
        # ── Ultrasonic Config ─────────────────────────────────
        # [แก้ไข] เพิ่มระยะเป้าหมายให้ห่างแปลงออกมาหน่อย กันล้อขูด
        self.US_TARGET_DIST = 0.04 
        
        # [แก้ไข] ถ้าล้อหน้ายังขูดแปลง ให้ลองเปลี่ยนค่า FRONT_OFFSET เป็น 0.0 หรือ 0.01
        ...
        # Vars สำหรับรับค่าจาก AprilTag (แปลงเป็นเมตร)
        ...
        # ── ตัวแปรสำหรับ Sequence เดิน-หยุด ───────────────────────
        self.sequence_steps = []
        ...
        self.get_logger().info(f"Robot Master: Sequence Mode (Blind Straight)")
    ...
    def vision_cb(self, msg):
        ...
        # อัปเดตค่าที่อ่านได้จาก AprilTag ทันทีที่เจอ (แปลงหน่วย cm เป็น m)
        if len(d) > 6:
            self.plant_dist_m     = d[4] / 100.0  # d[4] = plant_dist
            self.plant_gap_m      = d[5] / 100.0  # d[5] = plant_gap
            self.plant_interval_m = d[6] / 100.0  # d[6] = plant_interval
    ...
    def control_loop(self):
        ...
        # ── พักหลังจากเจอ Tag กล้องหน้า ก่อนหมุน ──
            ...
            if time.monotonic() - self.turn_start_time < 6.1: left_rpm, right_rpm = -40.0, 40.0
            else:
        ...
        # ── State ถอยหา Tag หลัง ──
            ...
            if elapsed_back < 2.0:
                ...
                if tag_ready:
                    # สร้าง Sequence ไดนามิกจากค่าที่อ่านได้
                    self.sequence_steps = [
                        self.plant_dist_m,     # จังหวะ 1: dist
                        self.plant_dist_m,     # จังหวะ 2: dist
                        self.plant_gap_m,      # จังหวะ 3: gap
                        self.plant_interval_m, # จังหวะ 4: interval
                        self.plant_interval_m, # จังหวะ 5: interval
                        self.plant_interval_m  # จังหวะ 6: interval
                    ]
        ...
        # ── State หยุดพัก 2 วินาทีก่อนเริ่มรัน Sequence ──
        ...
        # ── State เดินตาม Sequence ตาบอด ไม่ใช้เซนเซอร์ใดๆ ──
        ...
        # ── State หยุดพักระหว่าง Sequence ──
        elif self.state == STATE_PAUSE_SEQUENCE:
            left_rpm, right_rpm = 0.0, 0.0
            if time.monotonic() - self.sequence_pause_start_time > 2.0:
                self.total_dist = 0.0
