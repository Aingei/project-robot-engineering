...
class RobotMaster(Node):
    ...
    def control_loop(self):
        ...
        if self.state == STATE_SERVO1_FIRST:
            ...
            # 📌 Calculate New Angle Progress (0 to Target)
            ...
            if abs(yaw_error) < 3.0 or elapsed > 12.0:
                ...
                self.send_rpm(0, 0); self.forward_start_time = time.monotonic(); self.state = STATE_FORWARD_AFTER_TURN
            else:   
            ...
            duration = 3.5 if self.path_mode == 1 else 5.0
            if time.monotonic() - self.forward_start_time < duration:
                rpm = self._m_s_to_rpm(self.walk_speed); left_rpm, right_rpm = rpm, rpm
            ...
            else: left_rpm, right_rpm = 30.0, -30.0 
            ...
            if time.monotonic() - self.back_start_time > 2.0 and self.back_tag_found and self.back_tag_y_ratio < self.back_tag_threshold:
                ...
                self._set_scan_mode(0.0); self.blind_back_start_time = time.monotonic(); self.state = STATE_BACK_BLIND
        ...
        # --- Planting Logic ---
            ...
            if time.monotonic() - self.plant_timer > 2.0: self.plant_timer = time.monotonic(); self.state = STATE_MOVE_FWD
            ...
            if time.monotonic() - self.plant_timer > 2.0: self.current_servo3 = 0.0; self.plant_timer = time.monotonic(); self.state = STATE_S3_100
        elif self.state == STATE_S3_100:
            if time.monotonic() - self.plant_timer > 2.0: self.send_stepper_cmd(1.0, 800); self.plant_timer = time.monotonic(); self.state = STATE_S3_0
        elif self.state == STATE_S3_0:
            if time.monotonic() - self.plant_timer > 2.0: self.current_servo3 = 180.0; self.plant_timer = time.monotonic(); self.state = STATE_STP_UP2
        elif self.state == STATE_STP_UP2:
            if time.monotonic() - self.plant_timer > 4.0: self.plant_timer = time.monotonic(); self.state = STATE_MOVE_BWD
